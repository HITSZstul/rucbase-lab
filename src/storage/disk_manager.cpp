...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    off_t offset_of_file = page_no*PAGE_SIZE;
    if(lseek(fd, offset_of_file, SEEK_SET)==-1){
        throw InternalError("DiskManager::write_page ERROR:lseek failed");
    }
    ssize_t byte_written = write(fd, offset, num_bytes);
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    if(read(fd, offset, num_bytes)!=num_bytes){
        throw InternalError("DiskManager::write_page ERROR:read failed");
    }
}
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    if(unlink(path.c_str())==-1){
        throw FileNotFoundError("DiskManager::destroy_file ERROR: file destroying failed");
    }
}
...
int DiskManager::open_file(const std::string &path) {
    ...
    if(fd == -1){
        throw FileNotFoundError("DiskManager::open_file ERROR: file open failed");
    }
